# webpack-preset-help | пресет вебпак
...
1. [Инициализация git](#1.%20Инициализация%20git)
2. [Инициализация npm](#2.%20Инициализация%20npm)
3. [Установка webpack](#3.%20Устанавливаем%20webpack)
4. [Установка локального сервера](#4.%20установка%20локального%20сервера)
5. [SplitChunksPlugin - вырезает куски кода в один файл](#5.%20добавляем%20SplitChunksPlugin%20из%20коробки.%20Он%20вырезает%20одинаковые%20куски%20кода)
6. [Prefetching - ленивая загрузка файлов](#6.%20Prefetching%20или%20ленивая%20загрузка%20(для%20модулей%20которые%20должны%20быть%20загружены,%20но%20не%20использованы%20сразу))
7. [Плагин проверки бандла bundle Analasis](#7.%20По%20желанию%20bundle%20Analasis%20для%20проверок%20бандла)
8. [Очистка `.dist` и автоматическое создание html](#8.%20Очистка%20`./dist`%20и%20создание%20html)
9. [CSP - команда для дополнительной защиты от XSS](#9.%20CSP%20-%20команда%20для%20дополнительной%20защиты%20от%20XSS)
10. [three shaking и sideEffects - отрубание не исполняемого кода](#10.%20three%20shaking%20и%20sideEffects%20-%20отрубание%20не%20исполняемого%20кода%20(пока%20что%20не%20пробовал,%20чуть%20позже%20займусь)%20-%20в%20конечный%20код%20не%20добавлял)
11. [WebpackMerge - обьединяет наши конфиги в один](#11.%20WebpackMerge%20-%20обьединяет%20наши%20конфиги%20в%20один)
12. [Workbox - для работы сайта без интернета](#12.%20Workbox%20-%20для%20работы%20сайта%20без%20интернета)
13. [Public path - будет добавлено в скором будующем :)](#13.%20Public%20path%20-%20будет%20добавлено%20в%20скором%20будующем%20:))

- [Вывод](#Вывод)
- [Если кто-то разбирается](#Далее%20то%20в%20чем%20я%20не%20разобрался)
...
## 5. добавляем SplitChunksPlugin из коробки. Он вырезает одинаковые куски кода
...
## 6. Prefetching или ленивая загрузка (для модулей которые должны быть загружены, но не использованы сразу)
...
## 8. Очистка `./dist` и создание html
...
## 9. CSP - команда для дополнительной защиты от XSS
...
## 12. Workbox - для работы сайта без интернета
...
## 13. Public path - будет добавлено в скором будующем :)
...
## Далее то в чем я не разобрался
